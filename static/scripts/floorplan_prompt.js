...
window.FPPrompt = (function () {
  ...
  function v2LayerLines() {
    return [
      ...
      '- "props": simple 3D furnishings and scenery. Indoor types: pillar, crate, barrel, table, chair, chest, statue, stairs, rubble, bed, shelf, altar. Outdoor/nature types: tree, pine, dead_tree, bush, stump, log, boulder, campfire, tent, cactus, well, cart. Modern/sci-fi types: console, wreck, barricade. Graveyard types: gravestone, grave_cross, grave_mound, tomb, coffin, obelisk, mausoleum, iron_fence, gallows. Portal types (all walkable — tokens step through them): portal_arch (stone archway), portal_ring (standing ring), portal_circle (runic circle on the floor), portal_mirror, portal_gate (jagged fiery gate); place a "glow" light at the same x/y so it shines. x/y = center in grid squares; optional "rot" (degrees clockwise), "scale" (0.25-4, default 1). Props are solid and BLOCK token movement (stairs, rubble, bush, and grave_mound stay walkable) — never place one in a doorway or a 1-square corridor. A campfire is unlit geometry — pair it with a "brazier" light at the same x/y for its glow.',
