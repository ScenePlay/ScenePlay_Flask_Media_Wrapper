...
class Wled():
    ...
    def set_speed(self, _speed):
        ...
        state = {"seg": [{"sx": clip(int(_speed), 0 , 255)}]}
    ...
    def set_effect_pallette(self, effect, palette):
        """
        Sets the effect for the entire strip.
        ...
        Parameters:
            effect: index of effect to use from the get_effect() method
        """
        state = {"seg": [{"fx": int(effect), "pal": int(palette)}]}
        return self.set_state(state)
    
    def set_pallette_by_ID(self, palette):
        """
        Sets the effect for the entire strip.
    ...
    def set_effect_pallette_by_name_(self, effect_name, palette_name):
        """
        Sets the effect by name
        ...
        pallets = self.get_palettes()
        if palette_name in pallets:
            palette_index = pallets.index(palette_name)
        else:
            palette_index = 0
            #raise ValueError(f"{palette_name} not found in effects")

        return self.set_effect_pallette(effect_index, palette_index)
    ...
    def set_color(self, color,color2,color3):
        """
        ...
        state = {"seg": [{"col": [json.loads(color), json.loads(color2), json.loads(color3)]}]}
        return self.set_state(state)
